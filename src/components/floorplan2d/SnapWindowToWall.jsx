...
const SnapDoorWindowToWall = ({ stageRef }) => {
  ...
  function getWindowTransformFromSnap(snapInfo) {
    ...
    const windowHeight = Math.min(WINDOW_CONFIG.height, wallThickness); // 👈 tự động khớp

    const offset = ((point.x - v1.x) * dx + (point.y - v1.y) * dy) / wallLength;

    if (offset < windowLength / 2 || offset > wallLength - windowLength / 2) {
      return {
        valid: false,
        x: point.x,
        y: point.y,
  ...
  useEffect(() => {
    const handleMove = () => {
      ...
      if (snap?.type === "wall") {
        const transform = getWindowTransformFromSnap({
          wall: snap?.wall,
          point: snap?.point,
        });
        if (transform) {
    ...
    const handleClickAddWindow = (snapInfo, mode) => {
      ...
      const innerPolygon = [
        {
          x: center.x - dir.x * halfLen - perp.x * halfWindow,
          y: center.y - dir.y * halfLen - perp.y * halfWindow,
        },
        {
          x: center.x + dir.x * halfLen - perp.x * halfWindow,
          y: center.y + dir.y * halfLen - perp.y * halfWindow,
    ...
    const handleClick = () => {
      ...
      if (mode == 'window') {
        handleClickAddWindow(snapInfo, mode)
      }
    };
  ...
  const renderPreviewDoor = () => {
    ...
    return (
      <>
        ...
        >
          {WINDOW_CONFIG.renderSVG(snapInfo?.snapped && snapInfo?.valid ? "black" : "red")}
        </div>
      </>
    );
  };
